...
$width = Read-Host -Prompt "Desired width"
$height = Read-Host -Prompt "Desired height"

if ((Read-Host -Prompt "Proceed in the path $path to format all .jpeg, .jpg and .png files to the width of $width and height of $height? y/n") -eq 'y') {
    $files = Get-ChildItem -Path $path -Recurse -Include *.jpeg, *.png, *.jpg

    foreach ($file in $files) {
        Set-ImageSize -FullName $file.FullName -Destination $file.DirectoryName -WidthPx $width -HeightPx $height -Overwrite -Verbose -FixedSize
    }
}
